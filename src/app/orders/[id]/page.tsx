...
export default function OrderDetailPage() {
  ...
  const [formData, setFormData] = useState({
    ...
    propertyClass: "",
    satelliteImagePath: "",
    complicatingFactors: {} as Record<string, boolean>,
    lot: "",
    block: "",
    subdivision: "",
    researcherId: "",
  ...
  const handleToggleComplicatingFactor = async (factor: string) => {
    if (!order) return;
    const currentFactors = formData.complicatingFactors || {};
    const updated = {
      ...currentFactors,
      [factor]: !currentFactors[factor],
    };
    setFormData((prev) => ({ ...prev, complicatingFactors: updated }));
  ...
  const handleEnrichPropertyGIS = async () => {
    ...
    try {
      let res = await fetch(`/api/orders/${order.id}/enrich`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
      ...
      if (!res.ok) {
        res = await fetch(`/api/orders/${order.id}/enrich-property`, {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({
            address: formData.address || order.address,
      ...
      if (result.data || result.order) {
        const enriched = result.data || result.order;
        setFormData((prev) => ({
          ...prev,
          county: enriched.county || prev.county,
          parcelId: enriched.parcelId || enriched.taxParcelId || prev.parcelId,
          taxParcelId: enriched.taxParcelId || enriched.parcelId || prev.taxParcelId,
          deedAcres: enriched.deedAcres !== null && enriched.deedAcres !== undefined ? enriched.deedAcres : prev.deedAcres,
          acres: enriched.deedAcres !== null && enriched.deedAcres !== undefined ? enriched.deedAcres : (enriched.acres !== null ? enriched.acres : prev.acres),
          primaryOwner: enriched.primaryOwner || prev.primaryOwner,
          propertyClass: enriched.propertyClass || prev.propertyClass,
          satelliteImagePath: enriched.satelliteImagePath || prev.satelliteImagePath,
        }));
        setGisEnrichMessage({
          text: `Enrichment complete! Parcel ID: ${enriched.parcelId || enriched.taxParcelId || "Detected"}, Acres: ${enriched.deedAcres ?? enriched.acres ?? "N/A"}`,
        });
        await fetchOrder();
      }
    } catch (err: any) {
  ...
  const populateFormState = (data: OrderDetail) => {
    setFormData({
      ...
      zip: data.zip || "",
      county: data.county || "",
      parcelId: data.parcelId || data.taxParcelId || "",
      taxParcelId: data.taxParcelId || data.parcelId || "",
      deedAcres: data.deedAcres !== null && data.deedAcres !== undefined ? data.deedAcres : (data.acres !== null && data.acres !== undefined ? data.acres : ""),
      acres: data.deedAcres !== null && data.deedAcres !== undefined ? data.deedAcres : (data.acres !== null && data.acres !== undefined ? data.acres : ""),
      primaryOwner: data.primaryOwner || "",
      propertyClass: data.propertyClass || "",
      satelliteImagePath: data.satelliteImagePath || "",
      complicatingFactors: data.complicatingFactors && typeof data.complicatingFactors === "object" ? (data.complicatingFactors as Record<string, boolean>) : {},
      lot: data.lot || "",
      block: data.block || "",
  ...
  const handleOpenEmailModal = async () => {
    ...
    const combinedEmails = Array.from(
      new Set(
        [order.clientEmail, order.client?.email, formData.clientEmail]
          .filter(Boolean)
          .flatMap((e) => (typeof e === "string" ? e.split(",").map((x) => x.trim()) : []))
          .filter(Boolean)
      )
    ).join(", ");
    setEmailTo(combinedEmails);
  ...
  const handleSendEmail = async (e: React.FormEvent) => {
    ...
    if (!order) return;
    if (!emailTo.trim()) {
      setEmailModalError("Please enter at least one recipient email address.");
      return;
    }
    try {
            ...
            <div className="space-y-3">
              ...
              <div className="grid grid-cols-2 gap-3">
                ...
                <div>
                  ...
                  <input
                    type="text"
                    value={formData.parcelId || formData.taxParcelId}
                    onChange={(e) => {
                      handleInputChange("parcelId", e.target.value);
              ...
              <div className="p-3 bg-slate-50 dark:bg-slate-800/60 rounded-lg border border-slate-200 dark:border-slate-700/80 space-y-3">
                <div className="flex items-center justify-between">
                  ...
                  {Object.values(formData.complicatingFactors || {}).some(Boolean) && (
                ...
                <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-2">
                  {COMPLICATING_FACTORS_OPTIONS.map((factor) => {
                    const isChecked = Boolean(formData.complicatingFactors?.[factor]);
                    return (
                      <label
                        key={factor}
              ...
              <form onSubmit={handleSendEmail} className="space-y-4">
                <div>
                  ...
                  <input
                    ...
                    placeholder="client@example.com, closing@example.com"
                    className="w-full px-3 py-2 bg-slate-50 dark:bg-slate-800 border border-slate-300 dark:border-slate-700 text-slate-900 dark:text-slate-100 rounded-lg text-xs font-medium focus:bg-white dark:focus:bg-slate-800 focus:outline-none focus:ring-2 focus:ring-blue-500 font-mono"
                  />
                </div>
