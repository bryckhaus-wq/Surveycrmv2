          ...
          <div className="bg-white dark:bg-slate-900 rounded-xl shadow-sm border border-slate-200 dark:border-slate-800 p-6 space-y-5">
            ...
            <div>
              <div className="flex items-center justify-between mb-1">
                ...
                {pricingResult?.requiresReview && !priceOverrideUnlocked && (
                  <span className="inline-flex items-center text-[10px] font-bold text-amber-600 dark:text-amber-400">
                    <Lock className="w-3 h-3 mr-0.5" /> Locked (PM Review)
                  </span>
                )}
              </div>
              <div className="flex items-center space-x-2">
                <span className="text-3xl font-extrabold text-slate-900 dark:text-slate-100 block">
                  ${Number(quote.price).toFixed(2)}
                </span>
                {pricingResult?.requiresReview && (
                  <button
                    type="button"
                    onClick={() => setPriceOverrideUnlocked(!priceOverrideUnlocked)}
                    className="text-[11px] font-semibold text-blue-600 hover:underline flex items-center ml-auto"
                  >
                    {priceOverrideUnlocked ? (
                      ...
                      <>
                        <Unlock className="w-3 h-3 mr-1" /> Override Price
                      </>
                    )}
                  </button>
                )}
                ...
                <div className="space-y-2">
                  {pricingResult.mode === "price" && (
                    <div className="bg-emerald-50/80 dark:bg-emerald-950/40 border border-emerald-200 dark:border-emerald-800/80 rounded-lg p-3 space-y-2 text-xs">
                      ...
                      <button
                        type="button"
                        onClick={() => pricingResult.total && handleApplySuggestedPrice(pricingResult.total)}
                  ...
                  {pricingResult.mode === "route" && (
                    <div className="bg-amber-50 dark:bg-amber-950/40 border border-amber-300 dark:border-amber-700 rounded-lg p-3 space-y-1.5 text-xs text-amber-900 dark:text-amber-200">
                      ...
                      {pricingResult.assignedManager && (
                        <div className="text-[10px] text-amber-800 dark:text-amber-300 font-medium">
                          Routing to: {pricingResult.assignedManager} ({pricingResult.assignedRole})
                        </div>
                      )}
                    </div>
                  )}
